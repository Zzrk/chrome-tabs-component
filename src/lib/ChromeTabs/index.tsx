...
const ChromeTabs = (
  ...
  useEffect(() => {
    ...
    el?.addEventListener('tabAdd', ({ detail }) => {
      const key = detail.tabEl.getAttribute('data-key');
      setTabs([...tabs.map(tab => ({ ...tab, active: 'false' })), { key: String(tabs.length), icon: false, title: 'New tab' }]);
      onTabAdd(tabs, key);
    })
    el?.addEventListener('tabRemove', ({ detail }) => {
      const key = detail.tabEl.getAttribute('data-key');
      setTabs(tabs.filter(tab => tab.key !== key));
      onTabRemove(tabs, key);
    })
  }, [])
          ...
          tabs.map(tab => (
            <div className="chrome-tab" key={tab.key} data-key={tab.key} active={tab.active}>
              ...
              <div className="chrome-tab-content">
                <div className="chrome-tab-favicon" style={tab.icon ? { backgroundImage: `url(${tab.icon})` } : {}}></div>
